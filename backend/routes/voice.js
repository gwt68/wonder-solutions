...
case 'history_menu': {
    ...
    case 'broadcast_message_select': {
      ...
      let i = session.data.browse_index || 0;
        ...
        await updateSession(callSid, 'broadcast_message_select', { browse_index: i });
...
function broadcastSourcePrompt(twiml, retry = false) {
  const prefix = retry ? "Hmm, that's not one of the options. " : '';
  gatherDigits(twiml, `${BASE_URL}/voice/handle`,
    `${prefix}To record something new, press 1. To pick from what you've saved, press 2.`,
    { numDigits: 1 });
}
...
function broadcastMethodPrompt(twiml, retry = false) {
  const prefix = retry ? "Hmm, that's not one of the options. " : '';
  gatherDigits(twiml, `${BASE_URL}/voice/handle`,
    `${prefix}To use each contact's usual method, press 1. ` +
...
function broadcastWhenPrompt(twiml, retry = false) {
  const prefix = retry ? "Hmm, that's not one of the options. " : '';
  gatherDigits(twiml, `${BASE_URL}/voice/handle`,
    `${prefix}To send it right now, press 1. To schedule it, press 2.`,
    { numDigits: 1 });
}
...
function broadcastMessageBrowse(twiml, messages, index, retry = false) {
  const prefix = retry ? "Hmm, that's not one of the options. " : '';
...
async function broadcastConfirmPrompt(callSid, twiml, userId) {
  ...
  if (d.broadcast_method && d.broadcast_method !== 'assigned') {
    ...
    methodDesc = parts.length ? parts.join(', and ') : 'by their usual method';
  }
...
async function executeBroadcast(callSid, twiml, userId) {
  ...
  await updateSession(callSid, 'main_menu');
  gatherDigits(twiml, `${BASE_URL}/voice/handle`,
    'You can hang up now, or press star for the main menu.',
    { numDigits: 1, noNav: true });
}
...
function groupPickPrompt(groups, retry = false) {
  ...
  return `${prefix}Here are your groups, please select which to send this message to:\n${list}\n` +
    `Reply with one number, or several separated by commas (like 1,3).${navFooter(false)}`;
}

function groupPrefixMultiPrompt(groups, retry = false) {
  const prefix = retry ? "Sorry, reply with 1, 2, or 3.\n" : '';
  return `${prefix}You picked ${groups.length} groups. How should the group title work?\n` +
    `1. Each group gets its own title\n2. Use one title for all\n3. No title${navFooter(false)}`;
}

function groupPrefixWhichPrompt(groups, retry = false) {
  const prefix = retry ? "Sorry, reply with the number of the group.\n" : '';
  const list = groups.map((g, i) => `${i + 1}. ${g.name}`).join('\n');
  return `${prefix}Which title should all of them start with?\n${list}${navFooter(false)}`;
}
...
function stepBefore(currentStep, data) {
  ...
  if (currentStep === 'group_prefix_multi') return 'group_pick';
  if (currentStep === 'group_prefix_which') return 'group_prefix_multi';
...
async function promptForStep(step, data, userId) {
  switch (step) {
    ...
    case 'group_prefix_multi': return groupPrefixMultiPrompt(data.groups || []);
    case 'group_prefix_which': return groupPrefixWhichPrompt(data.groups || []);
  ...
  switch (session.step) {
    ...
    case 'individual_number': {
      ...
      return methodPrompt();
        ...
        case 'group_pick': {
      const pool_ = session.data.group_page || [];
      const picked = [];
      digits.split(/[,\s]+/).forEach((chunk) => {
        const g = pool_[parseInt(chunk, 10) - 1];
        if (g && !picked.some((p) => p.id === g.id)) picked.push(g);
      });
      if (!picked.length) return groupPickPrompt(pool_, true);

      const base = {
        target: 'group', groups: picked,
        group_id: picked[0].id, group_name: picked[0].name,
        prev_before_method: 'group_pick',
      };

      const prefixMode = await getGroupPrefixMode(userId);
      if (prefixMode === 'always') {
        await updateSmsSendSession(userId, fromPhone, 'method_select', { ...base, prefix_style: 'each' });
        return methodPrompt();
      }
      if (prefixMode === 'never') {
        await updateSmsSendSession(userId, fromPhone, 'method_select', { ...base, prefix_style: 'none' });
        return methodPrompt();
      }
      if (picked.length > 1) {
        await updateSmsSendSession(userId, fromPhone, 'group_prefix_multi', base);
        return groupPrefixMultiPrompt(picked);
      }
      await updateSmsSendSession(userId, fromPhone, 'group_prefix_confirm', base);
      return `Include "${picked[0].name}:" at the start of the message?\n1. Yes\n2. No${navFooter(false)}`;
    }

    case 'group_prefix_multi': {
      const groups = session.data.groups || [];
      if (digits === '1' || digits === '3') {
        await updateSmsSendSession(userId, fromPhone, 'method_select', {
          prefix_style: digits === '1' ? 'each' : 'none', prev_before_method: 'group_pick',
        });
        return methodPrompt();
      }
      if (digits === '2') {
        await updateSmsSendSession(userId, fromPhone, 'group_prefix_which', {});
        return groupPrefixWhichPrompt(groups);
      }
      return groupPrefixMultiPrompt(groups, true);
    }

    case 'group_prefix_which': {
      const groups = session.data.groups || [];
      const g = groups[parseInt(digits, 10) - 1];
      if (!g) return groupPrefixWhichPrompt(groups, true);
      await updateSmsSendSession(userId, fromPhone, 'method_select', {
        prefix_style: 'single', prefix_group_name: g.name, prev_before_method: 'group_pick',
      });
      return methodPrompt();
    }
...
async function executeSmsSend(session, userId, fromPhone, scheduledAt) {
    const { message_id, target, contact_id, method } = session.data;
  await clearSmsSendSession(userId, fromPhone);
  const zone = await getUserTimezone(userId);

  const people = await resolveTargetContacts(
    { broadcast_target: target, broadcast_contact_id: contact_id, broadcast_groups: session.data.groups || [] },
    userId
  );
  if (!people.length) return 'No recipients found for that selection.';
  ...
  try {
    for (const bucket of buckets) {
      ...
      if (bucket.name) msgId = await cloneMessageWithGroupPrefix(message_id, bucket.name, userId);
      const recipients = bucket.list.map((p) => (useSpecificMethod ? { contact_id: p.id, methods: [method] } : { contact_id: p.id }));
      const result = await createSendBatch({
    ...
    if (scheduledAt) {
      ...
      return `Scheduled for ${total} contact${total === 1 ? '' : 's'} at ${formatted}.`;
    }
    return `Message sent to ${total} contact${total === 1 ? '' : 's'}.`;
  } catch (err) {
